...
function ViewReviewModal({ isOpen, onClose, order }) {
  ...
  useEffect(() => {
    if (isOpen) {
      if (!order?._id) {
        console.error("Could not find order._id to fetch reviews.");
        setLoading(false);
        return;
      }
      setLoading(true);
...
// ==========================================
// 🧾 3. MAIN RECEIPT MODAL (THE MISSING PIECE!)
// ==========================================
export default function ReceiptModal({ selectedOrder, setSelectedOrder, onSubmitReviews }) {
  ...
  return (
    <>
        ...
        <div style={{ backgroundColor: 'white', width: '100%', maxWidth: '600px', borderTopLeftRadius: '24px', borderTopRightRadius: '24px', padding: '25px', paddingBottom: '40px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 -10px 40px rgba(0,0,0,0.2)' }}>
          ...
          {(selectedOrder.imageUrl || selectedOrder.parchiImage || selectedOrder.image) && (
            <div style={{ marginBottom: '20px', textAlign: 'center', backgroundColor: '#f8fafc', padding: '12px', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
              ...
              <img src={selectedOrder.imageUrl || selectedOrder.parchiImage || selectedOrder.image} alt="Uploaded Parchi" style={{ maxWidth: '100%', maxHeight: '180px', borderRadius: '12px', objectFit: 'contain', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }} />
            </div>
          )}
          ...
          <div style={{ marginBottom: '20px' }}>
            {selectedOrder.items && selectedOrder.items.length > 0 ? (
              ...
              <div style={{ textAlign: 'center', padding: '20px', color: '#64748b', fontStyle: 'italic' }}>Items are being added...</div>
            )}
          </div>
          ...
          <div style={{ background: 'linear-gradient(90deg, #f8fafc, #f1f5f9)', padding: '15px', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #e2e8f0' }}>
            ...
            <span style={{ fontSize: '1.4rem', color: '#10b981', fontWeight: '900' }}>{typeof selectedOrder.totalAmount === 'number' ? `₹${selectedOrder.totalAmount}` : selectedOrder.totalAmount}</span>
          </div>
          ...
          <div style={{ marginTop: '25px', display: 'flex', flexDirection: 'column', gap: '15px', alignItems: 'center' }}>
             ...
             {isDelivered && isAlreadyReviewed && (
               <button onClick={() => setIsViewReviewModalOpen(true)} style={{ width: '100%', padding: '14px', backgroundColor: '#fefce8', color: '#ca8a04', border: '1px dashed #fef08a', borderRadius: '12px', fontSize: '1rem', fontWeight: '800', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
      ...
      <OrderReviewModal isOpen={isReviewModalOpen} onClose={() => setIsReviewModalOpen(false)} order={selectedOrder} onSubmitReviews={onSubmitReviews} />
      <ViewReviewModal isOpen={isViewReviewModalOpen} onClose={() => setIsViewReviewModalOpen(false)} order={selectedOrder} />
    </>
  );
}
