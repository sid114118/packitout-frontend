...
export default function Header({ user }) {
  ...
  // Local state to instantly update the UI without reloading the whole app
  ...
  const handleFindShops = async () => {
    ...
    try {
      ...
      setShops(data);
      if (data.length > 0) setSelectedShopId(data[0]._id); // Auto-select the first one
    } catch (err) {
  ...
  const handleSaveShop = async () => {
    ...
    try {
      ...
      // 2. Update local storage so the rest of the app knows
      localStorage.setItem("packitout_user", JSON.stringify(updatedUser));
      
      // 3. Update the header visually and close the panel
      setActiveShopName(updatedUser.primaryShop?.name || "");
      setIsChanging(false);
      setShops([]); // Reset the search
    } catch (err) {
      console.log("Error saving shop", err);
    }
  };

  // Determine what text to show in the slim bar
  ...
  return (
    <header style={{ position: 'sticky', top: 0, zIndex: 1000, boxShadow: '0 4px 10px rgba(0,0,0,0.1)' }}>
      ...
      {/* 🟢 TOP ROW: Brand & Navigation (Slimmer Padding) */}
      ...
      {/* 🔍 EXPANDING SHOP SELECTOR (Only shows when "Change" is clicked) */}
      {isChanging && (
        <div style={{ backgroundColor: '#2f3640', padding: '15px', borderTop: '1px solid #3d566e', boxShadow: 'inset 0 4px 6px rgba(0,0,0,0.1)' }}>
          ...
          {/* Error Message */}
          {shops.length === 0 && pincode && !loadingShops && (
            <div style={{ color: '#fca5a5', fontSize: '0.85rem', textAlign: 'center', marginTop: '5px' }}>
...
// Reusable styling for the top icons
