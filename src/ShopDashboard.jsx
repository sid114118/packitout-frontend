...
export default function ShopDashboard({ user, onExit }) {
  ...
  // 🛡️ Safety Check: If 'user' is missing, show a loading message instead of crashing
  if (!user) {
  ...
  const fetchOrders = () => {
    fetch("https://darkslategrey-snail-415133.hostingersite.com/orders")
      .then(res => res.json())
      .then(data => {
        // We filter orders where the shopId matches this logged-in shop's ID
  ...
  return (
    <div style={{ backgroundColor: '#1e272e', minHeight: '100vh', fontFamily: 'sans-serif', color: 'white' }}>
      ...
      <div style={{ backgroundColor: '#2f3640', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid #05c46b' }}>
        <div>
          <h2 style={{ margin: 0, color: '#05c46b' }}>🏪 {user.name || "My Shop"}</h2>
      ...
      <div style={{ padding: '20px' }}>
        <h3 style={{ marginBottom: '20px' }}>📥 Incoming Parchis</h3>

        {loading ? (
          <p>📡 Connecting to server...</p>
        ) : orders.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '50px', backgroundColor: '#2f3640', borderRadius: '15px', border: '1px dashed #7f8fa6' }}>
            <p style={{ fontSize: '1.1rem', color: '#7f8fa6' }}>No active orders in {user.pincode} yet.</p>
          </div>
        ) : (
              ...
              <div key={index} style={{ backgroundColor: 'white', color: '#2f3640', padding: '20px', borderRadius: '15px', boxShadow: '0 4px 15px rgba(0,0,0,0.3)' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #eee', paddingBottom: '10px', marginBottom: '10px' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <span style={{ backgroundColor: '#f1f2f6', padding: '5px 10px', borderRadius: '5px', fontWeight: 'bold', fontSize: '0.75rem' }}>#{order._id.slice(-5).toUpperCase()}</span>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '2px solid #f1f2f6', paddingTop: '15px' }}>
                  ...
                  <button style={{ backgroundColor: '#05c46b', color: 'white', border: 'none', padding: '12px 20px', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer' }}>
                    Accept Order 📦
                  </button>
                </div>
              </div>
