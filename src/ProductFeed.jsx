...
export default function ProductFeed({ user, onAddToCart, selectedCategory, onClearCategory }) {
  ...
  // 👇 THE MAGIC: If a category is selected, ONLY show that category!
  if (selectedCategory) {
    // Flexible search: Checks if the DB category contains words from the clicked category
    const filteredItems = items.filter(item => {
      const dbCat = (item.category || "").toLowerCase();
      const searchCat = selectedCategory.toLowerCase();
      // Simple match strategy: if the database category matches the clicked text
      return dbCat.includes(searchCat) || searchCat.includes(dbCat);
    });

    return (
      <div style={{ padding: '0 15px', maxWidth: '1000px', margin: '0 auto', textAlign: 'left' }}>
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: '20px', gap: '15px' }}>
          ...
          <h2 style={{ margin: 0, color: '#0f172a', fontSize: '1.4rem' }}>{selectedCategory}</h2>
        </div>
        ...
        {filteredItems.length === 0 ? (
          <div style={{ padding: '40px', textAlign: 'center', color: '#64748b', backgroundColor: 'white', borderRadius: '12px', border: '1px dashed #cbd5e1' }}>
            No products found in "{selectedCategory}" yet!
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '15px' }}>
            {filteredItems.map(item => <ProductCard key={item._id} item={item} isCarousel={false} />)}
          </div>
        )}
      </div>
    );
  ...
  // 👇 IF NO CATEGORY IS SELECTED, SHOW THE HOME CAROUSELS!
  return (
